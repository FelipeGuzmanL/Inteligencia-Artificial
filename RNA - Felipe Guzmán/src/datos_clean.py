...
def Kmeans(dfcompleto):
    ...
    df_origin = dfcompleto.reset_index(drop = True) #se resetea los indices para que guardar las clases y no ser eliminadas con el drop (problema que surgió)
    ...
    comp_centroide1 = pd.DataFrame() #creamos un dataframe para almacenar las comparaciones del centroide 1
    comp_centroide2 = pd.DataFrame() #creamos un dataframe para almacenar las comparaciones del centroide 2
...
def KNN(k,t,e,td):
    ...
    for i in range(len(test)): #recorremos los 200 datos de las pruebas
        for j in range(len(train)): #recorremos los 800 datos de los entrenamientos
            dis = np.linalg.norm(test.iloc[i]-train.iloc[j]) #calculamos la distancia euclidiana con cada parametro de las pruebas y los entrenamientos
...
#                    print("Clase ",l,": ",total_datos.loc[m, "gusta o no gusta"])
...
#            print("La clase del parametro ",i,"es: 0")
...
#            print("La clase del parametro ",i,"es: 1")
...
#        print("Parametro ",i)
#    print("Aciertos: ",cont_aciertos)
#    print("Fallidos: ",cont_fallidas)
